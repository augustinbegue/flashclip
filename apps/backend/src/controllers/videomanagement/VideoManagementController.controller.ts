...
type AIResultKey = "emoji" | "engaging" | "subtitles" | "original";
type AIResultMap = Record<AIResultKey, string | null>;
...
export class VideoManagementController {
  ...
  /**
   * getVideoDetails   */
  async getVideoDetails(params: { videoId?: string }): Promise<Video> {
    try {
  ...
  /**
   * getAIResults   */
  async getAIResults(params: { videoId?: string }): Promise<Video> {
    try {
      const videoId = params.videoId?.trim();
      if (!videoId) {
        throw new Error("videoId is required");
  ...
  private async buildAIResults(videoId: string): Promise<AIResultMap> {
    ...
    for (const variant of AI_VARIANTS) {
      const key = variant.replace(".mp4", "") as AIResultKey;
